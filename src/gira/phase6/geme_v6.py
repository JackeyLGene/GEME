...
_ALPHABET = ["0","1","s","+","\u00d7","=","forall","exists","x","y","z","sub",
             "swap","pair","comm",           # E1: 代数对称
             "set","succ","empty","rank",   # E2: 归纳法
             "point","line","shape","parallel","angle","triangle",  # E3: 空间
             "fn","const"]                   # 回退槽
...
class Memory:
    ...
    def __init__(self,capacity=10,merge_thresh=None,cooccur_window=50,cooccur_thresh=0.25,
        ...
        self._step_counter=0  # monotonic step count (fix: was len(self._window))
        self._cooccur={}; self._assoc_frames=0
        self._chain_count=0; self.max_chains=max_chains  # cap chain explosion
        self._merge_dists=[]  # successful merge distances
        self._learn_dists=[]  # all nearest distances (bootstrap)
        
    def _adaptive_thresh(self):
        """Fully endogenous. Bootstrap: collect all nearest distances.
        Once 10+ samples, derive threshold from distribution (Q1).
        After bootstrap, use median of successful merge distances,
        floored to last_ok/2 (prevents zero-deadlock when identical
        vectors produce dist=0 → median=0 → no more merges).
        """
    ...
    def observe(self,vec,sig,src=""):
        ...
        # Collect all nearest distances for bootstrap
        ...
        # ── Step 2: merge (when threshold learned) or create (learning phase) ──
        if thresh is not None and bi>=0 and bd<=thresh:
            ...
            # Learning phase (thresh=None): accept all, uniform weight
        ...
        step_id = self._step_counter  # monotonic counter (fix: was len(self._window))
        ...
        for i in range(len(self._window)):
            for j in range(i+1,min(i+3,len(self._window))):
                ...
                if id1==id2: continue  # only skip identical self-pair
            ...
            for (sa,sb),count in list(self._cooccur.items()):
                ...
                if ratio>=self.cooccur_thresh and count>=max(5,total_steps*0.05):
                    ...
                    if existing:
                        ...
                        if len(base_vecs)>=2:
                            # Weighted average of ALL matching frames
                            ...
                            continue  # skip: not enough base frames for meaningful association vector
                        if len(self.frames)>=self.capacity:
                    ...
                    # Chain: shared element between associations (capped)
                    if existing and existing[0].weight>20 and self._chain_count<self.max_chains:
                        for other_f in self.frames:
                            ...
                            if "──" not in osig or "══" in osig: continue  # only chain concepts, not other chains
                            pa=set(assoc_sig.split("──")); pb=set(osig.split("──"))
                            if pa&pb and len(pa)>1:  # at least one side has 2+ distinct elements = cross-association
                                ms=assoc_sig+"══"+osig
                ...
                # No decay: window size limits _cooccur growth naturally
...
class GEME:
    ...
    def process_sig(self, formula, sig=None):
        """Process with explicit signature (bypass compute_signature).
        Used for experiments with custom function symbols (swap, succ, etc.)
        that compute_signature doesn't recognize.
        """
        self.frame_count+=1; self._input_count+=1
        if sig is None:
            sig = compute_signature(formula)
